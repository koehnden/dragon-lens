...
def _fetch_per_model_metrics(
    ...
    rows: list[dict] = []
    for model in models:
        data = fetch_json(endpoint, params={"vertical_id": vertical_id, "model_name": model})
        if not data:
            continue
        ...
        for item in data.get(items_key) or []:
            ...
            rows.append({
                ...
                "dvs": round(item["dragon_lens_visibility"] * 100),
            })
    return rows
...
def render_model_heatmap(
    ...
    pivot = df.pivot_table(index="entity", columns="model", values="dvs", fill_value=0)
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        zmin=0,
        zmax=100,
        colorbar=dict(title="DVS", ticksuffix=""),
        hoverongaps=False,
    ))

    fig.update_layout(
        title="Visibility Across LLMs",
        xaxis_title="",
        yaxis_title="",
