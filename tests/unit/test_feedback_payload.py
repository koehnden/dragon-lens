...
def test_build_brand_feedback_valid():
    ...
    assert build_brand_feedback(rows) == [{"action": "validate", "name": "Toyota", "reason": None}]


def test_build_brand_feedback_replace():
    rows = [{"name": "Toyta", "action": "wrong", "correct_name": "Toyota", "reason": "typo"}]
...
def test_build_mapping_feedback():
    ...
    missing = [{"product_name": "CRV", "brand_name": "Honda", "action": "add", "reason": "missing"}]
    assert build_mapping_feedback(existing, missing) == [
        {"action": "reject", "product_name": "RAV4", "brand_name": "Toyota", "reason": None},
        {"action": "add", "product_name": "CRV", "brand_name": "Honda", "reason": "missing"},
    ]
