...
from services.extraction.consultant import (
    _build_reverse_brand_map,
    _merge_suffix_variants,
    _partition_products_by_brand,
    _strip_brand_prefixes,
    _strip_product_suffix,
    _try_strip_brand,
)
...
class TestBuildReverseBrandMap:
    def test_includes_aliases_and_canonicals(self) -> None:
        ...
        result = _build_reverse_brand_map(aliases)
...
class TestTryStripBrand:
    def test_strips_latin_prefix(self) -> None:
        ...
        stripped, brand = _try_strip_brand("Salomon X Ultra 4 GTX", brands, reverse)
    ...
    def test_strips_chinese_prefix(self) -> None:
        brands = ["比亚迪"]
        reverse = {"比亚迪": "BYD"}
        stripped, brand = _try_strip_brand("比亚迪唐DM-i", brands, reverse)
    ...
    def test_no_match_returns_none(self) -> None:
        ...
        stripped, brand = _try_strip_brand("Merrell Moab 3", brands, reverse)
    ...
    def test_skips_short_brands(self) -> None:
        brands = ["X"]
        reverse = {"X": "X"}
        stripped, brand = _try_strip_brand("X Ultra 4", brands, reverse)
        assert stripped is None

    def test_short_remainder_rejected(self) -> None:
        brands = ["Salomon"]
        reverse = {"Salomon": "Salomon"}
        stripped, brand = _try_strip_brand("Salomon X", brands, reverse)
        assert stripped is None

    def test_digit_only_remainder_rejected(self) -> None:
        brands = ["Moab"]
        reverse = {"Moab": "Merrell"}
        stripped, brand = _try_strip_brand("Moab 3", brands, reverse)
        assert stripped is None
...
class TestStripBrandPrefixes:
    def test_strips_and_adds_mapping(self) -> None:
        ...
        new_aliases, new_map = _strip_brand_prefixes(
    ...
    def test_preserves_existing_mapping(self) -> None:
        ...
        _, new_map = _strip_brand_prefixes(
            product_aliases, product_brand_map, reverse,
        )
    ...
    def test_no_mutation_of_originals(self) -> None:
        ...
        _strip_brand_prefixes(original_aliases, original_map, reverse)
...
class TestStripProductSuffix:
    def test_strips_gtx(self) -> None:
        assert _strip_product_suffix("X Ultra 4 GTX") == "X Ultra 4"

    def test_strips_waterproof(self) -> None:
        assert _strip_product_suffix("Moab 3 Waterproof") == "Moab 3"

    def test_strips_multiple_suffixes(self) -> None:
        assert _strip_product_suffix("Anacapa Mid GTX") == "Anacapa"

    def test_no_suffix_returns_none(self) -> None:
        assert _strip_product_suffix("Moab 3") is None

    def test_single_word_returns_none(self) -> None:
        assert _strip_product_suffix("GTX") is None

    def test_strips_all_wthr(self) -> None:
        assert _strip_product_suffix("Lone Peak ALL-WTHR") == "Lone Peak"
...
class TestMergeSuffixVariants:
    def test_merges_gtx_variant_to_base(self) -> None:
        ...
        result = _merge_suffix_variants(aliases)
...
class TestPartitionProductsByBrand:
    def test_partitions_correctly(self) -> None:
        ...
        by_brand, unmapped = _partition_products_by_brand(aliases, brand_map)
